namespace Soap.PfBase.Api
{
    ...
    public static class DiagnosticFunctions
    {
        ...
        public static async Task OnOutputStreamReadyToBeWrittenTo<TPing, TPong, TSendLargeMsg, TReceiveLargeMsg, TIdentity>(
            ...
            ILogger logger) where TPing : ApiCommand, new() 
        ...
        private static async Task CheckBlobStorage(ApplicationConfig applicationConfig, IMessageAggregator messageAggregator,
            Func<string, ValueTask> writeLine, string functionHost)
        {
            
                var blobClient = new BlobStorage(new BlobStorage.Settings(applicationConfig.StorageConnectionString, messageAggregator));
